<x-layout bodyClass="g-sidenav-show  bg-gray-200">
    ...
    <main class="main-content position-relative max-height-vh-100 h-100 border-radius-lg ">
        <x-navbars.navs.auth titlePage="تعديل المستخدم"></x-navbars.navs.auth>
                                ...
                                <form method='POST' action='{{ route('admins.update' , $user->id) }}' enctype="multipart/form-data">
                                    ...
                                    <div class="row">
                                        ...
                                        @if($user->role->id == 3 || $user->role->id == 2)
                                            <div class="mb-3 col-md-6">
                                                <label class="form-label">{{ __('form.work_link') }}</label>
                                                <input type="text" name="work_link" class="form-control border border-2 p-2" value='{{ $user->work_link }}'>
                                                @error('work_link')
                                                <p class='text-danger inputerror'>{{ $message }} </p>
                                                @enderror
                                            </div>
                                            <div class="mb-3 col-md-6">
                                                ...
                                                <input type="text" name="description" class="form-control border border-2 p-2" value='{{ $user->description }}'>
                                                @error('description')
                                                <p class='text-danger inputerror'>{{ $message }} </p>
                                                @enderror
                                            </div>
                                        ...
                                        <div class="mb-3 col-md-6">
                                            ...
                                            <select name="status" class="form-control border border-2 p-2">
                                                <option value="active" @selected($user->status == 'active')>{{ __('form.statuses.active') }}</option>
                                                <option value="pending" @selected($user->status == 'pending')>{{ __('form.statuses.pending') }}</option>
                                                <option value="suspended" @selected($user->status == 'suspended')>{{ __('form.statuses.suspended') }}</option>
                                            </select>
                                            @error('status')
                                            <p class='text-danger inputerror'>{{ $message }} </p>
                                            @enderror
                                        ...
                                        <div class="mb-3 col-md-6">
                                            ...
                                            <select name="role" class="form-control border border-2 p-2">
                                                <option value="1" @selected($user->role_id == '1')>{{ __('form.roles.admin') }}</option>
                                                <option value="2"@selected($user->role_id == '2')>{{ __('form.roles.verified_author') }}</option>
                                                <option value="3"@selected($user->role_id == '3')>{{ __('form.roles.author') }}</option>
                                                <option value="4"@selected($user->role_id == '4')>{{ __('form.roles.user') }}</option>
                                            </select>
                                            @error('status')
                                            <p class='text-danger inputerror'>{{ $message }} </p>
                                            @enderror
